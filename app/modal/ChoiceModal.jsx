import { Modal, ModalBody, ModalHeader } from "flowbite-react";
import { useState } from "react";

const ChoiceModal=({
     openChoiceModal,
     setOpenChoiceModal,
    setChooseResumeType,
    setOpenRegisterModal,
})=>{
      const [selectedType, setSelectedType] = useState(null);

      const handleIndiVidual=()=>{
        setSelectedType('individual')
        setChooseResumeType('individual')
        setOpenChoiceModal(false)
        setOpenRegisterModal(true)
      }
      const handleOrganization=()=>{
        setSelectedType('organization')
        setChooseResumeType('organization')
        setOpenChoiceModal(false)
        setOpenRegisterModal(true)
      }
    return(
        <>
        <Modal size="6xl" show={openChoiceModal} onClose={() => setOpenChoiceModal(false)}>
                <ModalHeader className='border-none pb-0 absolute right-3 top-3 bg-transparent'>&nbsp;</ModalHeader>
                <ModalBody className='bg-white p-0'>
                     <div className=" flex items-center justify-center p-4">
      <div className=" rounded-xl max-w-2xl w-full shadow-2xl relative">
        {/* Close button */}
      

        {/* Header */}
        <div className="text-center pt-8 pb-6">
          <h2 className="text-2xl font-semibold text-gray-900 mb-2">Resume Type</h2>
        </div>

        {/* Content */}
        <div className="px-8 pb-8">
          <div className="grid grid-cols-2 gap-6">
            {/* Individual Resume Card */}
            <div
              className={`cursor-pointer rounded-lg border-2 transition-all duration-200 ${
                selectedType === 'individual'
                  ? 'border-purple-500'
                  : 'border-gray-200 hover:border-gray-300'
              }`}
              onClick={() => handleIndiVidual()}
            >
              {/* Image */}
              <div className="aspect-[4/3] bg-gray-100 rounded-t-lg overflow-hidden">
                <img
                  src="data:image/svg+xml;base64,"
                  alt="Individual Resume"
                  className="w-full h-full object-cover"
                />
              </div>
              
              {/* Content */}
              <div className="p-4 text-center">
                <h3 className="text-lg font-medium text-gray-900 mb-1">Individual Resume</h3>
              </div>
            </div>

            {/* Organization Resume Card */}
            <div
              className={`cursor-pointer rounded-lg border-2 transition-all duration-200 ${
                selectedType === 'organization'
                  ? 'border-purple-500'
                  : 'border-gray-200 hover:border-gray-300'
              }`}
              onClick={() => handleOrganization()}
            >
              {/* Image */}
              <div className="aspect-[4/3] bg-gray-100 rounded-t-lg overflow-hidden">
                <img
                  src="data:image/svg+xml;base64,"
                  alt="Organization Resume"
                  className="w-full h-full object-cover"
                />
              </div>
              
              {/* Content */}
              <div className="p-4 text-center">
                <h3 className="text-lg font-medium text-gray-900 mb-1">Organization Resume</h3>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
                </ModalBody>
            </Modal>
        </>
    )
}
export default ChoiceModal;