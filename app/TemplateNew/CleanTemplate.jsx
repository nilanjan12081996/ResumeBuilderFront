...
const CleanTemplate = ({ formData, sections, sectionOrder, resumeSettings }) => {
  ...
  // ── Layout ────────────────────────────────────────────────────────────────
  // topBottom = page top & bottom margin only
  // leftRight = page left & right margin only
  ...
  // Fixed internal column padding (NOT affected by topBottom/leftRight)
  ...
  const SectionHeading = ({ title }) => (
    <div style={{ marginBottom: `${betweenTitlesContent}pt` }}>
      ...
      <div style={{ width: "28pt", borderBottom: "2pt solid #111", marginTop: "3pt" }} />
    </div>
  );

  // ── SkillBar — table-based, PDF-safe ──────────────────────────────────────
  // Skills:    total=5, 0-based level → filled = level+1 (level 0 → 1 segment)
  // Languages: total=6, 0-based level → filled = level+1 (level 0 → 1 segment)
  const SkillBar = ({ level, total = 5 }) => {
    const lvl = Math.max(0, Math.min(total - 1, Math.round(level ?? 0)));
    const filled = lvl + 1; // ✅ always at least 1 segment visible
    const SEG_H = "4pt";

    // Build array of cells: filled=dark, rest=gray
  ...
  // Language string → 0-based number (0–5 for 6 segments)
  const getLangLevel = (langLevel) => {
    if (typeof langLevel === "number") return langLevel;
    const map = {
      "native": 5, "fluent": 4, "advanced": 3, "upper intermediate": 3,
      "intermediate": 2, "elementary": 1, "beginner": 1, "basic": 0,
  ...
  const renderInfoSidebar = () => {
    ...
    return (
      <div key="info" style={{ marginBottom: `${betweenSections}pt` }}>
        ...
        {formData.phone && (<><span style={labelStyle}>Phone</span><span style={valueStyle}>{formData.phone}</span></>)}
        {formData.email && (<><span style={labelStyle}>Email</span><a href={`mailto:${formData.email}`} style={{ ...valueStyle, wordBreak: "break-all", color: "#333", textDecoration: "none" }}>{formData.email}</a></>)}
        {formData.driving_licence && (<><span style={labelStyle}>Driving Licence</span><span style={valueStyle}>{formData.driving_licence}</span></>)}
        {formData.nationality && (<><span style={labelStyle}>Nationality</span><span style={valueStyle}>{formData.nationality}</span></>)}
        {formData.dob && (<><span style={labelStyle}>Date of Birth</span><span style={valueStyle}>{formData.dob}</span></>)}
        {formData.birth_place && (<><span style={labelStyle}>Place of Birth</span><span style={valueStyle}>{formData.birth_place}</span></>)}
      </div>
    );
  };
  ...
  // ════════════════════════════════════════════════════════════════════════
  //  MAIN — SCRATCH
  //  Date is BELOW the title (same as experience/education), NOT right-aligned
  // ════════════════════════════════════════════════════════════════════════
  ...
  const renderSummaryScratch = () => {
    ...
    return (
      <div key="summary" style={{ marginBottom: `${betweenSections}pt` }}>
        ...
        <div className="resume-content" style={{ ...bodyStyle, textAlign: "justify" }} dangerouslySetInnerHTML={{ __html: formData.summary }} />
      </div>
    );
  };
      ...
      <div key="employment" style={{ marginBottom: `${betweenSections}pt` }}>
        ...
        {formData.employmentHistory.map((job, i) => (
          <div key={i} style={{ marginBottom: "12pt" }}>
            ...
            {dateRange(job.startDate, job.endDate) && <span style={dateStyle}>{dateRange(job.startDate, job.endDate)}</span>}
            {job.description && <div className="resume-content" style={{ ...bodyStyle }} dangerouslySetInnerHTML={{ __html: job.description }} />}
          </div>
        ))}
      </div>
    );
      ...
      <div key="education" style={{ marginBottom: `${betweenSections}pt` }}>
        ...
        {formData.educationHistory.map((edu, i) => (
          <div key={i} style={{ marginBottom: "10pt" }}>
            ...
            {edu.description && <div className="resume-content" style={{ ...bodyStyle }} dangerouslySetInnerHTML={{ __html: edu.description }} />}
          </div>
        ))}
      </div>
    );
      ...
      <div key="activities" style={{ marginBottom: `${betweenSections}pt` }}>
        ...
        {formData.activityHistory.map((act, i) => (
          <div key={i} style={{ marginBottom: "10pt" }}>
            ...
            {act.description && <div className="resume-content" style={{ ...bodyStyle }} dangerouslySetInnerHTML={{ __html: act.description }} />}
          </div>
        ))}
      </div>
    );
  ...
  // ════════════════════════════════════════════════════════════════════════
  //  MAIN — FROM SECTIONS PROP
  // ════════════════════════════════════════════════════════════════════════

  const renderSummaryFromSection = (section) => (
    <div key={section.id} style={{ marginBottom: `${betweenSections}pt` }}>
      <SectionHeading title={section.title} />
      <div className="resume-content" style={{ ...bodyStyle, textAlign: "justify" }} dangerouslySetInnerHTML={{ __html: section.summary }} />
    </div>
  );
    ...
    <div key={section.id} style={{ marginBottom: `${betweenSections}pt` }}>
      ...
      {section.experiences?.map((exp, i) => (
        <div key={exp.id || i} style={{ marginBottom: "12pt" }}>
          ...
          {exp.description && <div className="resume-content" style={{ ...bodyStyle }} dangerouslySetInnerHTML={{ __html: exp.description }} />}
        </div>
      ))}
    </div>
  );
    ...
    <div key={section.id} style={{ marginBottom: `${betweenSections}pt` }}>
      ...
      {section.certifications?.map((cert, i) => (
        <div key={cert.id || i} style={{ marginBottom: "8pt" }}>
          ...
          {cert.description && <div className="resume-content" style={{ ...bodyStyle, marginTop: "2pt" }} dangerouslySetInnerHTML={{ __html: cert.description }} />}
        </div>
      ))}
    </div>
  );
    ...
    <div key={section.id} style={{ marginBottom: `${betweenSections}pt` }}>
      ...
      {section.internships?.map((intern, i) => (
        <div key={intern.id || i} style={{ marginBottom: "10pt" }}>
          ...
          {intern.description && <div className="resume-content" style={{ ...bodyStyle }} dangerouslySetInnerHTML={{ __html: intern.description }} />}
        </div>
      ))}
    </div>
  );
    ...
    <div key={section.id} style={{ marginBottom: `${betweenSections}pt` }}>
      ...
      {section.activities?.map((a, i) => (
        <div key={a.id || i} style={{ marginBottom: "10pt" }}>
          ...
          {dateRange(a.startDate, a.endDate) && <span style={dateStyle}>{dateRange(a.startDate, a.endDate)}</span>}
          {a.description && <div className="resume-content" style={{ ...bodyStyle }} dangerouslySetInnerHTML={{ __html: a.description }} />}
        </div>
      ))}
    </div>
  );

  // Skills sidebar — 5 segments
  ...
  // Custom advanced — date BELOW title
    ...
    <div key={section.id} style={{ marginBottom: `${betweenSections}pt` }}>
      ...
      {section.items?.map((item, i) => (
        <div key={item.id || i} style={{ marginBottom: "10pt" }}>
          ...
          {item.description && <div className="resume-content" style={{ ...bodyStyle, marginTop: "2pt" }} dangerouslySetInnerHTML={{ __html: item.description }} />}
        </div>
      ))}
    </div>
  );
            ...
            {history.map((item, i) => (
              <div key={i}>
                ...
                {!hideLevel && item.level !== undefined
                  ? <SkillBar level={item.level ?? 0} total={5} />
                  : <div style={{ marginBottom: "5pt" }} />
                }
              </div>
            ))}
  ...
  const renderMainContent = () => {
    ...
    const order = sectionOrder || ["summary", "employment", "education", "activities", "internships", "courses", "custom", "languages", "hobbies"];
    const scratchMap = {
  ...
  const renderSidebarContent = () => {
    ...
    const map = { skills: renderSkillsSidebar, languages: renderLanguagesSidebar, hobbies: renderHobbiesSidebar };
    return order.filter(id => SIDEBAR_TYPES.has(id)).map(id => map[id]?.() ?? null);
  };

  // ════════════════════════════════════════════════════════════════════════
  //  JSX
  //  Outer wrapper: padding = topBottom (top/bottom) + leftRight (left/right)
  //  Two-column layout: TABLE-based (PDF safe, no flexbox)
  //  Inner column padding: fixed SIDEBAR_PAD / MAIN_PAD (not affected by settings)
  // ════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div style={{ overflowY: "auto" }}>
      ...
      {/* ── Outer page wrapper — topBottom & leftRight apply here only ── */}
      <table style={{
        width: "100%",
        minHeight: "297mm",
        ...
        <tbody>
          {/* TOP MARGIN ROW */}
          <tr>
            <td style={{ height: `${topBottom}pt`, padding: 0 }} colSpan={3} />
          </tr>

          {/* CONTENT ROW: left margin | sidebar | main | right margin */}
          <tr>
            {/* Left margin */}
            <td style={{ width: `${leftRight}pt`, padding: 0, verticalAlign: "top" }} />

            {/* Inner content table: header + divider + two columns */}
            <td style={{ padding: 0, verticalAlign: "top" }}>

              {/* HEADER */}
              <div style={{ textAlign: headerTextAlign, marginBottom: `${Math.round(topBottom * 0.4)}pt` }}>
                <div style={{
                  fontFamily: secondaryFont,
              ...
              {/* HORIZONTAL RULE */}
              <div style={{ borderTop: "1pt solid #ccc", marginBottom: "0" }} />
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                ...
                <tbody>
                  <tr>
                    ...
                    <td style={{
                      verticalAlign: "top",
                      padding: `${SIDEBAR_PAD}pt ${SIDEBAR_PAD}pt ${SIDEBAR_PAD}pt 0`,
                    ...
                    <td style={{
                      verticalAlign: "top",
                      padding: `${MAIN_PAD}pt 0 ${MAIN_PAD}pt ${MAIN_PAD}pt`,
                    }}>
                      {renderMainContent()}
                    </td>
                  </tr>
