...
const LinkedInPersonalDetails = ({ register, watch, setValue }) => {
  ...
  // ✅ KEY FIX: crop save করার সময় base64 টা আলাদা field এ save করি
  // profileImage = server URL (preview এর জন্য)
  // profileImageBase64 = base64 data (DOCX embed এর জন্য)
  const handleProfileCropSave = async (base64) => {
    setProfileCropSrc(null);
    setProfileError("");
    // base64 টা এখনই save করি DOCX এর জন্য
    setValue("profileImageBase64", base64);
    try {
      ...
      if (fullUrl) {
        ...
        setValue("profileImageBase64", null);
      }
    } catch (err) {
    ...
    <div className="acco_section bg-white rounded-xl mb-1 overflow-hidden">
      ...
      <div className="relative" style={{ marginBottom: "90px" }}>
        ...
        <div className="relative group">
          ...
          <input type="file" id="linkedin-cover-upload" accept=".jpg,.jpeg,.png" className="hidden" onChange={handleCoverChange} disabled={coverLoading} />
          {coverImage && !coverLoading && (
            <div className="absolute top-2 right-2 flex gap-1">
              <label htmlFor="linkedin-cover-upload" className="cursor-pointer flex items-center gap-1 bg-black/50 hover:bg-black/70 text-white text-xs px-2 py-1 rounded-full transition">
                <FaCamera size={10} /> Change
              </label>
              <button type="button" onClick={() => { setValue("coverImage", ""); setValue("coverImageBase64", null); }}
        ...
        <div className="absolute left-5" style={{ bottom: "-90px" }}>
          <div className="flex flex-col items-center gap-1">
            ...
            <input type="file" id="linkedin-profile-upload" accept=".jpg,.jpeg,.png" className="hidden" onChange={handleProfileChange} disabled={uploadImageLinkedInLoading} />
            {profileImage && !uploadImageLinkedInLoading && (
              <button type="button" onClick={() => { setValue("profileImage", ""); setValue("profileImageBase64", null); }}
                className="flex items-center gap-1 text-xs text-gray-500 hover:!text-red-500 !bg-transparent">
                <MdDelete size={12} /> Remove
              </button>
            )}
            <p className="text-[10px] text-gray-400 text-center leading-tight max-w-[90px]">JPG, JPEG, PNG<br />Up to 1MB</p>
            {profileError && <p className="text-[10px] text-red-500 text-center leading-tight max-w-[110px]">{profileError}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="p-5 pt-2">
        ...
        <button type="button" onClick={() => setShowAdditionalDetails(!showAdditionalDetails)}
