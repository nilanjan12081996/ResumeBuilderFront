...
// import Image from 'next/image';
// import { IoCheckmarkOutline } from 'react-icons/io5';
// const CustomizeSection = ({ selectedTemplate, onSelectTemplate, themeColor, setThemeColor }) => {
//   const [activeTab, setActiveTab] = useState('Template & Colors');
...
//             key={tab}
//             onClick={() => setActiveTab(tab)}
//             className={`px-12 py-4 text-lg font-medium transition-all border-b-4 ${activeTab === tab ? 'border-blue-500 text-slate-900' : 'border-transparent text-slate-400'
//               }`}
//           >
//             {tab}
//           </button>
...
//                 className={`w-10 h-10 rounded-full border-2 flex items-center justify-center ${selectedColor === color ? 'border-blue-500' : 'border-gray-200'
//                   }`}
//               >
//                 <div className={`w-7 h-7 rounded-full bg-${color}-600 ${color === 'black' ? 'bg-black' : ''}`} />
//                 {selectedColor === color && color !== 'black' && <span className="text-white text-xs">✓</span>}
//               </button>
//             ))}
//           </div>
//         </div> */}
...
//                 className={`relative w-10 h-10 rounded-full border-2 flex items-center justify-center ${selectedColor === color.value
//                     ? 'border-blue-500'
//                     : 'border-gray-300'
//                   }`}
//               >
...
//               key={f}
//               onClick={() => setFilter(f)}
//               className={`px-6 py-2 rounded-full border text-sm font-medium transition-all ${filter === f ? 'bg-blue-500 border-blue-500 text-white' : 'bg-white border-gray-300 hover:border-blue-500'
//                 }`}
//             >
//               {f}
//             </button>
...
//           {templates.map((template) => (
//             <div key={template.id} className="flex flex-col items-center">
//               <h3 className="text-xl font-medium mb-4 text-slate-800">{template.name}</h3>
//               <div
//                 className={`relative bg-white p-2 border shadow-sm transition-all cursor-pointer hover:shadow-xl hover:-translate-y-1 ${selectedTemplate === template.id ? 'ring-2 ring-blue-500 border-blue-500' : 'border-gray-200'
//                   }`}
//                 onClick={() => onSelectTemplate(template.id)}
//               >
//                 <Image
...
//                 {selectedTemplate === template.id && (
//                   <div className="absolute inset-0 flex items-center justify-center bg-black/30">
//                     <div className="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center text-white text-2xl font-bold shadow-lg">
//                       <IoCheckmarkOutline />
//                     </div>
//                   </div>
//                 )}
...
//                 {/* PDF/DOCX BADGES */}
//                 <div className="absolute bottom-4 right-4 flex gap-1">
//                   <span className="bg-orange-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded uppercase">PDF</span>
//                   <span className="bg-orange-400 text-white text-[10px] font-bold px-1.5 py-0.5 rounded uppercase">DOCX</span>
//                 </div>
//               </div>
//             </div>
//           ))}
