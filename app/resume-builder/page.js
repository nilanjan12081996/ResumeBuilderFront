...
import { addCountResume, addCountResumeOrg, saveAchivmentInfo, saveCertificatesInfo, saveEducationInfo, saveForDraft, saveLanguageInfo, savePersonalInfo, saveProjectInfo, saveSkillInfo, saveTemplate, saveWorkExp, saveResumeNew, uploadImageScratch } from '../reducers/ResumeSlice';
...
import SimpleCustomSection from './SimpleCustomSection';
import AdvancedCustomSection from './AdvancedCustomSection';
...
const DynamicSimpleCustomSection = ({ sectionId, register, watch, control, setValue, removeSection }) => {
  const { fields, append, remove, move } = useFieldArray({
    control,
    name: `customSimpleHistory_${sectionId}`,
  });
...
// ── Dynamic wrapper for AdvancedCustomSection (own useFieldArray) ──
const DynamicAdvancedCustomSection = ({ sectionId, register, watch, control, setValue, removeSection }) => {
  const { fields, append, remove, move } = useFieldArray({
    control,
    name: `customAdvancedHistory_${sectionId}`,
  });

  return (
    <AdvancedCustomSection
      sectionId={sectionId}
      register={register}
      watch={watch}
      control={control}
...
const {uploadImageScratchLoading} = useSelector((state) => state.resume);
  ...
  const getSteps = () => {
    ...
    activeSections.forEach((sectionId) => {
      ...
      } else if (typeof sectionId === 'string' && sectionId.startsWith('custom_simple_')) {
        steps.push({ id: steps.length + 1, title: watch(`customSimpleTitle_${sectionId}`) || 'Custom Section (Simple)', sectionId: sectionId });
      } else if (typeof sectionId === 'string' && sectionId.startsWith('custom_advanced_')) {
        steps.push({ id: steps.length + 1, title: watch(`customAdvancedTitle_${sectionId}`) || 'Custom Section (Advanced)', sectionId: sectionId });
      }
    });
                        ...
                        STEPS.find(s => s.id === step)?.isAddSectionStep && (
                          ...
                          <AddSection
                            ...
                            onSelectSection={(sectionId) => {
                              if (['courses', 'hobbies', 'extra_curricular', 'languages', 'internships'].includes(sectionId)) {
                                if (!activeSections.includes(sectionId)) {
                                  setActiveSections([...activeSections, sectionId]);
                                }
                              } else if (sectionId === 'custom_simple') {
                                const BASE = 'Custom Section (Simple)';
                                const existingTitles = activeSections
                                  .filter(s => s.startsWith('custom_simple_'))
                                  .map(s => watch(`customSimpleTitle_${s}`) || BASE);
                                ...
                                const newId = `custom_simple_${Date.now()}`;
                                setActiveSections([...activeSections, newId]);
                                setTimeout(() => setValue(`customSimpleTitle_${newId}`, uniqueTitle), 50);
                              } else if (sectionId === 'custom_advanced') {
                                const BASE = 'Custom Section (Advanced)';
                                const existingTitles = activeSections
                                  .filter(s => s.startsWith('custom_advanced_'))
                                  .map(s => watch(`customAdvancedTitle_${s}`) || BASE);
                                let uniqueTitle = BASE;
                                let counter = 1;
                                while (existingTitles.includes(uniqueTitle)) {
                                  uniqueTitle = `${BASE} ${counter}`;
                                ...
                                const newId = `custom_advanced_${Date.now()}`;
                                setActiveSections([...activeSections, newId]);
                                setTimeout(() => setValue(`customAdvancedTitle_${newId}`, uniqueTitle), 50);
                              }
                            }}
                          />
                        </div>
