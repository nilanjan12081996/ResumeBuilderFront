...
const Page = () => {
  ...
  const handlePlanClick = (e, pln) => {
    ...
    if (hasActiveSubscription() && thisPrice < activePrice) {
      toast.error(
        "You already have an active plan. Please cancel your current subscription before downgrading.."
      );
      return;
    }
  ...
  const handlePaymentModal = async (e, data) => {
    ...
    const isUpgrade = hasActiveSubscription() && newPlanPrice > activePrice;
    ...
    try {
      ...
      if (isUpgrade) {
        ...
        if (!upgradePlanOrder.fulfilled.match(upgradeRes)) {
          toast.error(
            upgradeRes?.payload?.message ||
              "Failed to create upgrade order."
          );
          return;
        }
        ...
        const options = {
          ...
          currency: data.currency,
          name: "ResumeMile",
          description: `Upgrade to plan ${data.plan_id}`,
        ...
        if (!createOrder.fulfilled.match(result)) {
          toast.error(
            result?.payload?.message || "Failed to create order."
          );
          return;
        }
          ...
          <div className="max-w-6xl mx-auto">
                      ...
                      <div className="ml-3">
                        ...
                        <div className="mt-2 text-sm text-green-700">
                          <p>
                            <strong>Plan:</strong>{" "}
                            {
                              getCurrentActiveSubscription().Plan
                                ?.plan_name
                            }
                          </p>
                          <p>
                            ...
                            {new Date(
                              getCurrentActiveSubscription().end_date
                            ).toLocaleDateString()}
                          </p>
                          <p>
            ...
            <div className="subscription_tab_section inner_plan">
              <Tabs>
                ...
                {parsed?.signup_type_id == 1 && (
                  <>
                    {/* Build → plan_frequency = 1 */}
                    <TabPanel>
                      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 bg-white rounded-4xl p-5">
                        {plans?.data
                          ?.filter(
                            (pln) => pln?.plan_frequency === 1
                          )
                          .map((pln, i) => (
                            <div key={i}>{renderPlanCard(pln)}</div>
                          ))}
                      </div>
                    </TabPanel>

                    {/* Break Through → plan_frequency = 3 */}
                    <TabPanel>
                      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 bg-white rounded-4xl p-5">
                        {plans?.data
                          ?.filter(
                            (pln) => pln?.plan_frequency === 3
                          )
                          .map((pln, i) => (
                            <div key={i}>{renderPlanCard(pln)}</div>
                          ))}
...
// 1st option problem width dowangrade
// "use client";
